...
class UserMiddleware {
    ...
    static validFindUserByUsername = (req: Request, res: Response, next: NextFunction) => {
        const { username } = req.query;
        const errors: Record<string, string> = {};
        if (!this._regexUsername.test(username as string)) errors.username = "Nhập sai Họ và Tên";
        if (Object.keys(errors).length > 0) {
            return res.status(400).json({ valid: false, errors });
        }
        next();
    }
    static validUpdateAccount = (req: Request, res: Response, next: NextFunction) => {
        const { username, password, email, newPassword } = req.body;
        const errors: Record<string, string> = {};
        if (username && !this._regexUsername.test(username)) errors.username = "Nhập sai Họ và Tên";
        if (email && !this._regexEmail.test(email)) errors.email = "Nhập sai Email";
        if (password && !this._regexPassword.test(password)) errors.password = "Nhập sai Mật khẩu";
        if (newPassword) {
            if (!this._regexPassword.test(newPassword)) errors.newPassword = "Nhập sai Mật khẩu mới";
            else if (password && newPassword && password === newPassword) errors.newPassword = "Mật khẩu mới không được trùng với mật khẩu cũ";
        }
        if (Object.keys(errors).length > 0) {
            return res.status(400).json({ valid: false, errors });
        }
