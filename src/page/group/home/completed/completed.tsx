...
const CompletedTask = () => {
  ...
  useEffect(() => {
    ...
    socket.on("remove-completed-task", (idTask: string) => {
      ...
      addAlert({ title: "Thông báo", message: "Nhiệm vụ đã bị xóa", status: "error" });
    });

    socket.on("has-del-task", (data: string) => {
      setListTask((prev) => prev ? prev.filter((t) => t._id !== data) : []);
    ...
    socket.on("add-completed-task", (task: PropsViewsTask) => {
      ...
      addAlert({ title: "Thành công", message: "Có nhiệm vụ mới hoàn thành", status: "success" });
    });
