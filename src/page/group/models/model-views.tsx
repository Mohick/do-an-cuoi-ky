      ...
      >
        ...
        <div className="relative bg-[#141414] border-b border-[rgba(255,185,0,0.1)] px-5 py-[18px] flex flex-col gap-2">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[11px] uppercase tracking-[0.08em] text-[rgba(255,185,0,0.55)]">
          ...
          <button
            onClick={() => navigate(-1)}
            className="absolute top-[14px] right-4 w-7 h-7 rounded-full border border-[rgba(255,185,0,0.2)] bg-[rgba(255,185,0,0.06)] text-[#ffb900] flex items-center justify-center text-[13px] opacity-80 hover:opacity-100 transition-opacity"
          >
            <CloseOutlined />
          </button>
        </div>
        ...
        <div className="p-5 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="grid grid-cols-2 gap-[10px]">
            <InfoCard label="Người tạo">
              <span className="flex items-center gap-1.5">
                <UserOutlined className="text-[rgba(255,185,0,0.5)] text-[12px]" />
                {item.creator.username}
              </span>
            </InfoCard>
            <InfoCard label="Người nhận">
              <span className="flex items-center gap-1.5">
                <UserOutlined className="text-green-500/60 text-[12px]" />
                {item.implementer?.username ?? "Chưa có"}
              </span>
            </InfoCard>
            <InfoCard label="Deadline">
              <span className="flex items-center gap-1.5 text-[#ffb900]">
          ...
          <div>
            ...
            <div className="bg-[#1a1a1a] border border-white/5 rounded-[10px] p-3 text-[13px] text-[#bbb] leading-relaxed max-h-[100px] overflow-y-auto">
          ...
          <div>
            ...
            <div className="bg-[#1a1a1a] border border-white/5 rounded-[10px] px-3 py-[10px] flex items-center gap-2 text-[12px]">
              ...
              {item.url ? (
                <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-blue-400 underline break-all">
                  Bấm vào đây
                </a>
              ) : (
                <span className="text-white/25">Không có</span>
              )}
            </div>
          </div>
          ...
          <div>
                  ...
                  <div key={cmt._id} className="flex flex-col gap-1">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        ...
                        {cmt.alert && (
                          <span className="text-[9px] font-bold text-red-400 bg-[rgba(239,68,68,0.12)] border border-[rgba(239,68,68,0.2)] px-[6px] py-[1px] rounded-full">NEW</span>
                        )}
                      </div>
            ...
            {/* Input */}
            <div className="flex items-center gap-2 mt-2 bg-[#1a1a1a] border border-[rgba(255,185,0,0.15)] rounded-[10px] px-[10px] py-[6px]">
              <input
                type="text"
                value={comment}
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && comment) {
                    getCommentInTaskAPI({ id_task: item._id, comment, userID: data?.data.user._id || "", id_group: id_group || "" });
                    setComment("");
                  }
                }}
              ...
              <button
                ...
                className="w-7 h-7 rounded-[7px] bg-[rgba(255,185,0,0.12)] border border-[rgba(255,185,0,0.2)] text-[#ffb900] flex items-center justify-center cursor-pointer hover:opacity-75 transition-opacity"
              >
        ...
        <div className="border-t border-white/[0.06] bg-[#0e0e0e] px-5 py-3 flex gap-2 justify-end flex-wrap">
          {statusKey === "waiting" && (
            <ComponentButton page={statusKey} valid={true} onClick={() => claimTaskAPI({ id_task: id_task || "", id_group: id_group || "" })} name="Nhận" />
          )}
          {statusKey === "handling" && <>
