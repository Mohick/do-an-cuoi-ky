  ...
  return (
    <div className="w-full bg-app flex justify-center items-center py-10 px-2 min-h-screen">
      <div className="max-w-[400px] w-9/12 bg-black p-5 space-y-5 rounded-md">
          ...
          <div className=" bg-gray-100/50 mt-2 w-full rounded-md">
            <div className="relative flex w-full text-center">
              ...
              <button
                ...
                className={`flex-1 py-2 z-10 transition-colors duration-300 ${
                  !isLogin
              ...
              {/* Khối chạy qua lại */}
              <div
                className={`absolute top-0 left-0 h-full w-1/2 rounded-md bg-white transition-transform duration-300 ease-in-out ${
        ...
        {/* Form */}
        <AnimatePresence>{!isLogin ? <SignIn /> : <SignUp />}</AnimatePresence>
        <Link to="/" className="text-white block capitalize text-center">
          về trang chủ
        </Link>
      </div>
    </div>
